...
- `roadmap/mvp-roadmap.md` — Phase 0–20 sequence, tech-demo critical path, completed Phase 10 entry singleton correction, dependencies, non-goals, exit gates, and current Phase 11 scope.
...
The repository is currently in **Phase 11 — Categories and configurable Relevance execution**. The governing Category/Relevance predicate, precedence, default-Category, prospective-edit, pre-admin configuration, accounting, and validation semantics are defined in the domain, Source/collection, testing, and roadmap documents above.
