...
└── validation/   # created when durable observed validation artifacts are useful
```
...
- `contracts/testing-and-validation-contract.md` — project-wide regression law, evidence levels, test isolation, CI, PostgreSQL/fixture/browser/live-Source validation, and phase/prompt completion gates.
...
A roadmap phase is complete only when its documented exit gate and the inherited requirements in `contracts/testing-and-validation-contract.md` are actually verified against the final source tree. Do not infer completion from partial implementation, source inspection, stale earlier test results, or silently skipped suites.
