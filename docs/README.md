...
The accepted `1.0.0` customer launch established the first supported production schema/data baseline. The current development package is `1.1.0`, the active roadmap is Phase 1 — Distribution Profile foundation, and terminal release qualification after Phase 7 promotes the final validated `1.7.x` candidate directly to `2.0.0`.
...
Do not run the old `p1-0` stack or infer versions from the retired frontend-centric roadmap. Normal implementation planning now resumes with `/prompt-ass` → `/prompt-plan` → `/prompt-write p1-1` for Phase 1.
