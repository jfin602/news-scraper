...
- `roadmap/mvp-roadmap.md` — Phase 0–21 sequence, tech-demo critical path, completed Phase 10 entry singleton correction and Phases 10–17, dependencies, non-goals, exit gates, current Phase 18 HTML collection, Phase 20 production-baseline handoff, and terminal Phase 21 codebase simplification/maintainability hardening.
...
Phase 16 — True duplicate detection and grouping — is complete with durable validation in `validation/phase-16-true-duplicate-detection-and-grouping.md`. Phase 17 — Article and duplicate moderation — is complete with durable GREEN validation in `validation/phase-17-article-and-duplicate-moderation.md`. The repository is currently in **Phase 18 — Configurable HTML collection**. Phase 18 is a bounded static HTML parser-adapter extension for approved/whitelisted endpoints: it produces the shared Raw-item contract, reuses the existing downstream pipeline, keeps Source admission phrases RSS/Atom-only, and does not add browser collection. See the domain, Source/collection, architecture, admin, and testing contracts for authority.

The remaining documented roadmap after the current Phase 18 work is Phase 19 reliability/observability/production operations → Phase 20 customer launch validation → **Phase 21 codebase simplification and maintainability hardening**. Phase 20 acceptance establishes the production data/schema baseline and feature freeze; Phase 21 closes that freeze and is terminal unless the repository owner explicitly approves a later roadmap extension. Deferred ideas do not implicitly create Phase 22.
