...
- `contracts/public-feed-and-admin-contract.md` — Phase 8 public-feed backend/read model plus completed-MVP public feed, search/filtering, themes, external links, and Cloudflare-protected admin UX/change history.
...
Phase 0 documentation alignment, Phase 1 Application foundation, Phase 2 Database foundation, Phase 3 Publication and Source configuration, Phase 4 Collection eligibility and network safety, Phase 5 RSS/Atom transport, parsing, and minimal Collection runs, Phase 6 Article normalization, and Phase 7 Default Relevance/Article identity/persistence are complete with durable closeout validation. The repository is currently in Phase 8 — Basic public-feed backend.
