...
### Costs
- Administrators maintain Source/endpoint configuration and state.
...
A change violates this ADR when it contacts unapproved, archived, paused, or disabled configuration; widens endpoint policy beyond Source approval silently; follows a redirect without pre-request revalidation; accepts unsafe/unapproved Article links; or makes HTML/browser collection the default despite an adequate structured feed.
