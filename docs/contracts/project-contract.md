...
The completed 2.0 contracts define Distribution Profile selectors/lifecycle, the permanent v1 machine API, machine authentication, generic PHP synchronization/cache behavior, link/SEO limits, telemetry, and managed integration behavior. The owner-approved 3.0 roadmap is active in Phase 2 at package `2.2.0`; Phase 1 Profile-grounded Gemini digest foundation is complete and owner-accepted, the active Phase 2 hardens the PHP integration/customer package before production Gemini-capable package replacement, and later phases add Profile-grounded chat, multi-feed proof, and final admin/PHP integration tightening without weakening the existing 2.0 boundaries. WordPress, RSS/Atom, Linux VPS/Docker Compose self-host packaging, native self-host admin authentication, and autonomous public self-host production readiness remain outside the current commitment unless explicitly promoted.
