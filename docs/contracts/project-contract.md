...
The completed 2.0 contracts define Distribution Profile selectors/lifecycle, the permanent v1 machine API, machine authentication, generic PHP synchronization/cache behavior, link/SEO limits, telemetry, and managed integration behavior. The owner-approved 3.0 roadmap is active in Phase 1 at package `2.1.0`; it adds Profile-grounded Gemini assistance, a dedicated PHP integration correction/customer package refresh phase before production Gemini deployment, later Profile-grounded chat, multi-feed proof, and final admin/PHP integration hardening without weakening the existing 2.0 boundaries. WordPress, RSS/Atom, Linux VPS/Docker Compose self-host packaging, native self-host admin authentication, and autonomous public self-host production readiness remain outside the current commitment unless explicitly promoted.
