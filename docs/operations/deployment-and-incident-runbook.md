...
3. Check that `NODE_ENV`, `NEWS_SCRAPER_DATABASE_URL`, Web host/port, and deployment-specific process/origin configuration exist. Production distribution requires `NEWS_SCRAPER_WEB_TRUSTED_PROXY=loopback` plus `NEWS_SCRAPER_DISTRIBUTION_TRANSPORT=trusted_proxy_https`: run the Node origin behind one local TLS terminator that sends exactly one client `X-Forwarded-For` and `X-Forwarded-Proto: https`. Do not set these for direct/public or multi-hop proxy topologies; the Web process fails closed rather than trusting their headers. Development/test defaults to local HTTP. When admin is intended, confirm `NEWS_SCRAPER_ADMIN_ENABLED=true`. Check presence only; never print environment dumps, URLs, tokens, cookies, or credentials.
...
10. Through authorized Cloudflare Access, load `/admin`, Operations, Sources/endpoints, Distribution Profiles, Distribution Credentials, and a harmless read. Do not automate an authorized Access identity in the shared validator.
...
Distribution Profiles, the canonical distribution read-model producer, the machine credential/authentication/security foundation, the permanent v1 HTTP API with its production HTTPS/trusted-proxy configuration, the generic PHP synchronization/LKG core, and Phase 6 normalized local read/customer SSR integration are implemented. Phase 6 has local PHP/server/browser proof only; it is not managed external/customer-host/HostGator/cPanel/public-network/deployment evidence. Phase 7 real managed external integration and 2.0 release qualification are current work, and the final managed-integration checks below remain required until observed. Managed deployment validation expands without creating a separate deployment topology.
