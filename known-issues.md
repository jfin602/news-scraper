...
- Validation planning uses two explicit dimensions: a change-aware Test Necessity Matrix and a Test Environment Matrix. Required evidence resolves to `RUN`, `DEFER`, or `N/A` before execution; `DEFER` means required later in the correct environment, not skipped, passed, waived, or optional.
...
- The ordinary local `npm test` / `npm run check` path should be genuinely portable, while PHP runtime and PHP-backed browser evidence remain separately invokable for VPS validation. Database/browser/live-provider/deployment evidence should be classified by actual current prerequisites rather than globally assumed to be VPS-only.
- VPS-required evidence is **not** a default per-prompt, per-phase, or per-correction closeout gate. Ordinary implementation and ordinary phase/correction closeouts may finish GREEN when their applicable local `RUN` evidence is green and VPS evidence is explicitly deferred.
- Deferred VPS evidence is promoted to `RUN` at an explicitly designated full-system/project release qualification gate, normally against the final integrated candidate at the end of a major development line. Exact-tree cross-environment matching applies to that qualification candidate rather than requiring a VPS receipt for every intermediate prompt commit.
- Only a task/gate explicitly designated for VPS/full-system/release qualification, or an owner-approved exceptional gate, should require VPS evidence immediately.
